...
public class ReservationManagement implements IReservationManagement, Serializable {
        ...
        try
        {
            ...
            if (selectedFlight.getTotalSeats().length != bookedSeatsIndex)
            {
                ...
                do
                {
                    ...
                    //set Reservation ID
                    newReservation.setReservationID(this.reservation.size());
                    //set Flight ID
    ...
    public void checkin(FlightManagement flightManagement) {
        ...
        do
        {
            seatChoice = Input.inputNonBlankStr("Choose the seat number: ");
            for (Seat i : selectedFlight.getTotalSeats())
            {
                // if seat is empty
                if (i.getSeatName().equalsIgnoreCase(seatChoice))
                {
                    if (!i.isOccupied())
                    {
                        // set the seat number to reservation
                        i.setOccupied(true);
